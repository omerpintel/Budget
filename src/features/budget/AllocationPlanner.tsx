...
import { Copy, PiggyBank, Sparkles, TriangleAlert, Wand2 } from 'lucide-react';
...
import { coverDeficits, spreadByWeight } from '@/services/budget/engine';
...
export function AllocationPlanner({
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const [lines, incomes, allowances, people, actuals, history] = await Promise.all([
      ...
      return { lines, incomes, allowances, people, actuals, history };
    },
  });

  if (!data) return null;
  const { lines, incomes, allowances, people, actuals, history } = data;
  ...
  const plannedTotal =
    ...
    people.reduce((s, p) => s + allowanceOf(p.id), 0);
  const unassigned = income - plannedTotal;
